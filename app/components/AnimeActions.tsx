...
export default function AnimeActions({ animeSlug, animeTitle, animePoster }: AnimeActionsProps) {
  ...
  return (
    <div className="flex gap-3">
      <button
        ...
        className={`px-4 py-2.5 rounded-xl text-sm transition-all border flex items-center justify-center gap-2 group ${
          favorited
            ...
            : "bg-bg-card hover:bg-white/10 text-white border-white/5"
        }`}
      >
        <svg
          className={`w-4 h-4 transition-colors ${favorited ? "text-red-400" : "text-periwinkle group-hover:text-white"}`}
      ...
      <button
        onClick={handleShare}
        className="px-4 py-2.5 bg-bg-card hover:bg-white/10 text-white rounded-xl text-sm transition-all border border-white/5 flex items-center justify-center gap-2 group"
      >
        {copied ? (
          <>
