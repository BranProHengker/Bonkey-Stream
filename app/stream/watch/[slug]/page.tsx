  ...
  if (loading && !episode) {
    return (
      <div className="min-h-screen bg-bg-dark text-white flex flex-col">
        <Navbar />
        <div className="grow w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6">
          {/* Skeleton Video Player */}
          <div className="w-full aspect-video bg-white/5 animate-pulse rounded-2xl"></div>
          {/* Skeleton Title & Meta */}
          <div className="space-y-4 max-w-2xl mt-6">
            <div className="h-8 bg-white/5 animate-pulse rounded-lg w-3/4"></div>
            <div className="h-4 bg-white/5 animate-pulse rounded-lg w-1/3"></div>
          </div>
          {/* Skeleton Servers */}
          <div className="flex gap-3 mt-8">
            <div className="h-10 bg-white/5 animate-pulse rounded-xl w-24"></div>
            <div className="h-10 bg-white/5 animate-pulse rounded-xl w-24"></div>
            <div className="h-10 bg-white/5 animate-pulse rounded-xl w-24"></div>
          </div>
        </div>
      ...
      <div className="relative container mx-auto px-4 py-24 max-w-7xl">
        ...
        <div className="space-y-6">
          ...
          {episode.download_urls && episode.download_urls.length > 0 && (
            <details className="group bg-bg-card backdrop-blur-xl rounded-2xl border border-white/5 overflow-hidden shadow-xl">
              {/* Header */}
              <summary className="bg-white/5 px-5 sm:px-6 py-5 cursor-pointer list-none [&::-webkit-details-marker]:hidden flex items-center justify-between hover:bg-white/10 transition-colors">
                <h3 className="text-base sm:text-lg font-bold text-white flex items-center gap-3 tracking-tight">
                  <div className="w-10 h-10 bg-white/5 border border-white/10 rounded-xl flex items-center justify-center">
                    <svg className="w-5 h-5 text-white group-open:text-indigo transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center transition-colors">
                  <svg className="w-5 h-5 text-periwinkle/70 group-open:rotate-180 transition-transform duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div className="divide-y divide-white/5 border-t border-white/5">
                {episode.download_urls.map((group, idx) => {
                  ...
                  return (
                    <div key={idx} className="px-5 sm:px-6 py-5 hover:bg-white/[0.02] transition-colors group/item">
                      {/* Quality Header */}
                      ...
                      <div className="flex flex-wrap gap-2.5">
                        {group.links.map((link, linkIdx) => (
                          <a
                            ...
                            className="px-4 py-2 text-xs font-semibold bg-bg-dark hover:bg-white hover:text-bg-dark text-periwinkle rounded-lg transition-all duration-300 border border-white/10 hover:border-transparent hover:shadow-[0_0_15px_rgba(255,255,255,0.2)] flex items-center gap-2"
                          >
